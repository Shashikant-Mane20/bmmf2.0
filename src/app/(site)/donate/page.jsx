  ...
  const initiatePayment = () => {
    ...
    const options = {
      ...
      handler: async (response) => {
        ...
        // Ensure prefill data exists before using it
        const donorName = response.prefill?.name || "Anonymous";
        const donorEmail = response.prefill?.email || "donor@example.com";
        const donorContact = response.prefill?.contact || "9999999999";

        toast.success("Thank you for your contribution!"); // Show success toast

        // Send donation details to the API
        try {
